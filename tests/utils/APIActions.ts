...
export class APIActions {
  ...
  async verifyDataIntegrity(
    ...
    await allure.step(
      this.generateAssertionMessage(actualResponse.length, "Response Body"),
      async () => {
  ...
  private validateEntry(
    ...
    for (const key of allKeys) {
      allure.step(`${key} is ${responseEntry[key] as string[]}`, async () => {
        expect
          .soft(
        ...
        if (actualType === null || expectedType === null) {
          ...
          expect.soft(typeof actualType, message).toBe(expectedType);
        }
      });
    }
  }
